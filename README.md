...
| 类别 | 能力 |
| --- | --- |
| 浏览 | 分页读取目录、进入文件夹、可越过当前根目录继续返回上级、当前页搜索、九列元数据展示和按原始值排序 |
| 缓存 | 最多缓存 32 个最近目录；普通导航优先使用缓存，`F5` 强制刷新 |
| 写入 | 新建远程文件夹；一个入口混选文件和文件夹并递归上传；同名文件确认后覆盖 |
...
| 多语言 | 简体中文、English、Français、Русский；运行时即时切换并持久化 |
...
3. 编辑服务地址后，根目录历史会立即隐藏，但当前根目录仍可继续编辑。点击“加载根目录”会强制验证该地址；只有加载成功后，服务地址与根目录才会保存成历史记录。
4. 双击文件夹进入目录，双击文件打开对应预览。
5. 使用“新建文件夹”“上传文件或文件夹”或“保存到本地”执行写入和下载；上传选择器支持 Ctrl/Shift 混选。
...
- 模型窗口采用 CAD 的 Z-up 坐标约定和 Onshape 风格虚拟轨迹球：右键拖动时模型会跟随
  指针进行不受约束的三维旋转；Ctrl+右键或中键拖动平移、滚轮缩放。默认使用 Onshape
  建模视图相同的正交投影，右键双击根据包围盒和视口尺寸恢复最佳等轴测视角；
  ISO/TOP/FRONT/RIGHT 可快速切换标准视图。
- 场景包含渐变背景、四点式明暗光照、环境遮蔽、双层主次网格，以及使用相机四元数逆变换
  严格同步的世界坐标轴。左键暂未启用网格级点/线/面选择。
